...
class SquashCourtBookingMinorFunctions {
  String checkBookingConflicts(DateTime selectedStartingTime,
    ...
    for (var booking in existingBookings) {
      ...
      if ((existingStartingTime.isBefore(selectedEndingTime) &&
        ...
        return 'Squash Court is booked from ${DateFormat('h:mm a').format(existingStartingTime)} to ${DateFormat('h:mm a').format(existingEndingTime)} on ${DateFormat('dd MMMM yyyy').format(existingStartingTime)}. Please choose another time range'; // Conflict detected
      }
    }
    return ''; // No conflict
  }
...
class SquashCourtBookingMainFunctions {
  ...
  Future<List> addEntry(
    ...
    Completer<List> completer = Completer<List>();
      ...
      if (bookingMinorFunctions.checkTimeDiffValid(startingDateTime)) {
        ...
        firestore
            .collection(firestoreBookSquashCourtCollection)
            .get()
            .then((snapshot) {
